...
func (abi *ABI) EventById(idBytes []byte) (*Event, error) {
	if len(idBytes) < 4 {
		return nil, fmt.Errorf("data too short (%d bytes) for abi event lookup", len(idBytes))
	}
	for _, event := range abi.Events {
		if bytes.Equal(event.id(), idBytes) {
			return &event, nil
		}
	}
	return nil, fmt.Errorf("no event with id: %#x", idBytes)
}
