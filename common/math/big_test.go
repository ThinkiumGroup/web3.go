...
func TestPrintBigInt(t *testing.T) {
	tests := []struct {
		v *big.Int
		r string
	}{
		{v: parseStringToBigInt("28274000"), r: "2.8274e7"},
		{v: parseStringToBigInt("-28274000"), r: "-2.8274e7"},
		{v: parseStringToBigInt("34245"), r: "34245"},
		{v: parseStringToBigInt("-34245"), r: "-3.4245e4"},
		{v: parseStringToBigInt("987654"), r: "9.87654e5"},
		{v: parseStringToBigInt("1999000000900000000"), r: "1.9990000009e18"},
		{v: parseStringToBigInt("1000000000000000000"), r: "1e18"},
		{v: parseStringToBigInt("1111111111111111111"), r: "1.111111111111111111e18"},
		{v: parseStringToBigInt("0"), r: "0"},
	}
	for _, test := range tests {
...
func TestInt64MulRat(t *testing.T) {
	tests := []struct {
		i64    int64
		r      *big.Rat
		result int64
	}{
		{10, new(big.Rat).SetFrac64(1, 3), 4},
		{3, new(big.Rat).SetFrac64(1, 3), 1},
		{4, new(big.Rat).SetFrac64(1, 3), 2},
		{4, big.NewRat(1, 100), 1},
		{88, big.NewRat(1, 1), 88},
		{0, big.NewRat(2, 3), 0},
	}

	for _, test := range tests {
		if r := Int64MulRat(test.i64, test.r); r != test.result {
			t.Errorf("Int64MulRat(%d, %s) = %d, expecting: %d", test.i64, test.r, r, test.result)
		}
	}
}
